               ...
               .equ  	   TIMER,             0xFFFEC600 	  // Hardware timer
               .equ	    KEY_BASE,           0xFF200050		// base code for KEY
               .equ     LED_ADDRESS,        0xFF200000    // LEDs address
...
CONFIG_PRIV_TIMER:
                //MOV      R3, #1               // Increment to next bit
                //STR      R3, [R1, #0x12]      // Reset the interrupt status                             
                LDR      R1, =TIMER           
                LDR      R3, =49999999        // 200MHz/4 = 50000000 for 0.25 sec interval
                STR      R3, [R1]             // load value into the register
                MOV      R3, #0b111           // write to the control register
                STR      R3, [R1, #0x8]      
...
CHECK:          CMP      R5, #73              // Check if the interrupt were key presses
                BEQ      KEY_ISR              // Branch to KEY interrupt service routine if equal 
                CMP      R5, #29              // Check if the interrupt was the clock
                BEQ      PRIV_TIMER_ISR       // Branch to TIMER interrupt service routine if equal
 ...
 ***************************************************************************************/
                .global  KEY_ISR
KEY_ISR:        // reset the edge capture
                LDR      R0, =KEY_BASE
 ...
 *****************************************************************************/
 ...
 /// Have issues with this subroutine
                .global    TIMER_ISR
PRIV_TIMER_ISR: PUSH     {R8,R9,LR}
                LDR      R0, =TIMER
                STR      R3, [R0, #0xC]      // Reset the interrupt status
                LDR      R8, COUNT           // Get the value of the counter in the R8 register
                LDR      R9, RUN             // Get the value of RUN in ther R9
                ADD      R8, R9               // Increment count by the run value
                STR      R8, COUNT            // writes the value back to the count
                POP      {R8, R9, PC}
                MOV      PC, LR
